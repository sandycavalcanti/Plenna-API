...
function decodeBase64UrlBytes(value) {
    ...
    if (bytes.length === 0)
        throw new GmailAttachmentError('Attachment vazio');
 ...
 */
export async function downloadGmailAttachment(accessToken, messageId, metadata) {
    const { attachmentId, mimeType } = assertSupportedMetadata(metadata);
    if (!messageId.trim())
        throw new GmailAttachmentError('messageId ausente');
    let response;
    try {
        response = await axios.get(`https://gmail.googleapis.com/gmail/v1/users/me/messages/${encodeURIComponent(messageId)}/attachments/${encodeURIComponent(attachmentId)}`, { headers: { Authorization: `Bearer ${accessToken}` } });
    }
    catch {
    ...
    if (typeof encodedData !== 'string')
        throw new GmailAttachmentError('Resposta do Gmail sem dados');
