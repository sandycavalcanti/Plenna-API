...
function findLabeledAmount(text, labels) {
    ...
    for (let index = 0; index < lines.length; index += 1) {
        if (!labels.test(normalize(lines[index])))
            continue;
...
function extractItems(text) {
    ...
    for (const line of text.split(/\r?\n/).map((item) => item.trim()).filter(Boolean)) {
        ...
        if (!match || /(?:DESCRICAO|PRODUTO|QUANTIDADE|VALOR|TOTAL|FRETE|DESCONTO)/i.test(match[1]))
            continue;
...
function extractPayment(text) {
    ...
    for (const line of text.split(/\r?\n/)) {
        if (!/(?:FORMA\s+DE\s+PAGAMENTO|PAGAMENTO)/.test(normalize(line)))
            continue;
        const upper = normalize(line);
        if (/PIX/.test(upper))
            methods.add('Pix');
        if (/CARTAO\s+DE\s+CREDITO/.test(upper))
            methods.add('Cartao de credito');
        if (/CARTAO\s+DE\s+DEBITO/.test(upper))
            methods.add('Cartao de debito');
        if (/BOLETO/.test(upper))
            methods.add('Boleto');
    }
 ...
 */
export function detectDanfe(text) {
...
export function parseDanfeText(text) {
    if (!detectDanfe(text))
        return null;
...
export async function extractPdfText(bytes) {
    if (!Buffer.isBuffer(bytes) || bytes.length === 0)
        throw new DanfePdfParseError('PDF vazio');
    let parser = null;
    try {
        parser = new PDFParse({
        ...
        if (parser) {
            ...
            catch { /* libera recursos sem substituir o erro principal */ }
        }
    }
}
export async function parseDanfePdf(bytes) {
    let extracted;
    try {
        extracted = await extractPdfText(bytes);
    }
    catch (error) {
        if (error instanceof DanfePdfParseError)
            return { status: 'INVALID_PDF', danfe: null };
        throw error;
    }
    if (extracted.status === 'TEXT_LAYER_NOT_AVAILABLE')
        return { status: extracted.status, danfe: null };
