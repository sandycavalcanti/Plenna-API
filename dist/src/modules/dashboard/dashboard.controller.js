...
export class DashboardController {
    static async findGastosPorCategoria(req, res) {
        try {
            if (!req.userId) {
    ...
    static async findGastosPorFormaPagamento(req, res) {
        try {
            if (!req.userId) {
                return res.status(401).json({ error: 'Token inválido' });
            }
    ...
    static async findImpulsividade(req, res) {
        try {
            if (!req.userId) {
                return res.status(401).json({ error: 'Token inválido' });
            }
    ...
    static async findComprasAcimaLimite(req, res) {
        try {
            if (!req.userId) {
                return res.status(401).json({ error: 'Token inválido' });
            }
    ...
    static async findTempoVsGasto(req, res) {
        try {
            if (!req.userId) {
                return res.status(401).json({ error: 'Token inválido' });
            }
