import type { ExtractedPurchase, ExtractedPurchaseItem, ExtractionEvidence, NormalizedEmail } from './email-contracts.js';
...
type EvidenceInput = {
  field: ExtractionEvidence['field'];
  rawLabel: string;
  context: string;
};
...
function sourceText(email: NormalizedEmail) {
  return [email.textBody, email.subject, email.snippet].filter((value): value is string => Boolean(value?.trim())).join('\n');
}
...
function addEvidence(evidence: ExtractionEvidence[], input: EvidenceInput) {
  evidence.push({
    field: input.field,
    source: 'EMAIL_TEXT',
    confidence: null,
...
function findSemanticMoney(text: string, labels: string[]) {
...
function extractPaymentMethod(text: string) {
  ...
  for (const method of methods) {
    ...
    if (match) return { name: method.name, context: match[0] };
  }

  return null;
}

function cleanSenderName(from: string | null) {
  if (!from) return null;
...
function isItemLabel(line: string) {
...
function isProductStart(line: string) {
...
function lineValue(lines: string[], index: number, labels: string[]) {
  ...
  if (sameLineValue) return { value: sameLineValue, label, context: lines[index] };

  const nextLine = lines[index + 1]?.trim();
  if (nextLine && !isItemLabel(nextLine)) return { value: nextLine, label, context: `${lines[index]} ${nextLine}` };
  return null;
}

function findLabeledMoney(lines: string[], start: number, labels: string[], limit = 16) {
  for (let index = start; index < Math.min(lines.length, start + limit); index += 1) {
    ...
    if (amount !== null) return { amount, label: candidate.label, context: candidate.context };
  }
  return null;
}
...
function findQuantity(lines: string[], start: number, limit = 16) {
  for (let index = start; index < Math.min(lines.length, start + limit); index += 1) {
    ...
    if (Number.isFinite(quantity) && quantity > 0) return { quantity, label: candidate.label, context: candidate.context };
  }
  return null;
}

function extractItems(text: string, evidence: ExtractionEvidence[]): ExtractedPurchaseItem[] {
  const lines = text
    .split(/\r?\n/)
    .map((line) => line.trim())
    .filter(Boolean);
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (!name || /^R\$?\s*[\d.,]+$/i.test(name) || isItemLabel(name)) continue;
...
function extractEstablishment(email: NormalizedEmail, text: string, evidence: ExtractionEvidence[]) {
...
export class EmailPurchaseExtractor {
  static extract(email: NormalizedEmail): ExtractedPurchase {
    ...
    if (total) addEvidence(evidence, { field: 'totalAmount', rawLabel: total.label, context: total.context });
    if (order) addEvidence(evidence, { field: 'orderNumber', rawLabel: 'pedido', context: order.context });
    if (payment) addEvidence(evidence, { field: 'paymentMethod', rawLabel: payment.name, context: payment.context });
...
export function extractPurchase(email: NormalizedEmail) {
